...
attribute is `DecisionDriven.HotPathAttribute`, generated by the analyzer
package and citing a decision, and `eng/HotPathAttribute.cs` is retired; the
rule is `VARVE0003`/`VARVE0004`, not `VARVE0006`. Marking while writing, an
internal attribute and matching by full name stand.
