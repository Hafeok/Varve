...
### A defect this session caused, and the fix

**The commit introducing `eng/issue-refs.cs` closed an unrelated issue by
documenting how issue references work.** Its message explains why the gate
ignores the subject line and illustrates it with a quoted `"Closes #12"`, where
`#12` was meant as a placeholder for any number. GitHub reads a closing keyword
and a number reaching the default branch as an instruction, regardless of
quotation marks or surrounding prose, so the Operability milestone
([#12](https://github.com/Hafeok/Varve/issues/12)) was closed the moment the
merge landed.

Reopened, with the cause recorded on the issue. `main` is never rewritten, so
the commit keeps its text; the warning lives in the gate's banner instead —
which is now the second time this file's own documentation has fought its
toolchain, after the `<auto-generated` marker made the compiler treat the
licence gate as generated code.

### Both new rules confirmed empirically, after the merge

The follow-up fix above was **pushed directly to `main`**, which is the first
exercise of [ADR 0032](../adr/0032-trunk-based-development.md) and the first
evidence that the decision is operable rather than aspirational. It also
produced the clearest possible confirmation of
[ADR 0034](../adr/0034-commit-signing-and-the-sandbox-exception.md), from the
remote itself:
...
remote:   1cfad6636ca35d267e71cb52045c623560bd432a
9a823ff..1cfad66  main -> main
```

Two facts, neither of which was available when those ADRs were written:

- **A direct push to `main` is accepted.** Ruleset 1's required status checks
  did not block it, so trunk-based development works here as ADR 0032 assumes.
- **The signature bypass is real, and it is loud.** The commit *is* signed and
  GitHub still counts it as a violation, because the key is unknown to it —
  precisely the `unknown_key` situation ADR 0034 records. The App's bypass is
  what admits it, and the remote announces the bypass rather than hiding it,
  which is the best possible outcome for a stated deviation: it is visible
