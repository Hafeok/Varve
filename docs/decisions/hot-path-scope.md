...
- UTF-8 is validated and decoded with `System.Text.Unicode.Utf8` and
  `System.Text.Rune`;
- a guard clause is `ArgumentOutOfRangeException.ThrowIfNegative` or
...
**`CallerBufferWriterIsTheSink`.** `SparqlResultsWriter` writes either into a
stream through a pooled buffer, or into an `IBufferWriter<byte>` the caller
supplies. In the second case every write asks that buffer writer for a span and
advances it. That is an interface call into the caller's code on the hot path,
and it is the whole point of taking a buffer writer. The two private members
that make the calls cite this. The alternative is a generic writer over
`TWriter : IBufferWriter<byte>`, which changes the public constructors.
